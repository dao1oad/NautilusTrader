...
当前后端 contract 仍只定义 `health`、`overview` 与最小 WS invalidation / connection-state 语义；`#13` 新增的是浏览器侧多页面 shell、query key 和 shared page-state，不在此阶段定义最终静态资源托管或桌面交付流。
...
4. `overview.updated` / `snapshot.invalidate` 事件先进入 invalidation bus，再由 query client 触发 overview query 失效与重拉
