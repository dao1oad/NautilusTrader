...
随着 `scripts/prepare-remote-pr.ps1` 接管 issue worktree 的本地交接，当前仓库实际生成并校验的 issue 级 review 记录路径已经稳定为 `workspace/handoffs/review-resolution-issue-<issue>.md`；旧的 `local-review-issue-*.md` 只剩历史 PR 证据用途。

### Decision

把当前 canonical 的本地 pre-PR review 信号明确收敛到 `workspace/handoffs/review-resolution-issue-<issue>.md`，并在 truth-doc / runbook / pre-PR gate 中统一使用这一命名。

### Impact

review 交接产物、`pr-gate` 校验和 issue packet close-out 说明指向同一个文件模式；历史 `local-review-issue-*.md` 记录继续保留，但不再作为当前流程的权威输入。
